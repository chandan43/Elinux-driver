...
{
	lcd_pin_setup(gpbank->LCD_RS_PIN_NUMBER);
	lcd_pin_setup(gpbank->LCD_E_PIN_NUMBER);

	lcd_pin_setup(gpbank->LCD_DB4_PIN_NUMBER);
	lcd_pin_setup(gpbank->LCD_DB5_PIN_NUMBER);
	lcd_pin_setup(gpbank->LCD_DB6_PIN_NUMBER);
	lcd_pin_setup(gpbank->LCD_DB7_PIN_NUMBER);
}
...
{
	lcd_pin_release(gpbank->LCD_RS_PIN_NUMBER);
	lcd_pin_release(gpbank->LCD_E_PIN_NUMBER);

	lcd_pin_release(gpbank->LCD_DB4_PIN_NUMBER);
	lcd_pin_release(gpbank->LCD_DB5_PIN_NUMBER);
	lcd_pin_release(gpbank->LCD_DB6_PIN_NUMBER);
	lcd_pin_release(gpbank->LCD_DB7_PIN_NUMBER);
}
...
static void lcd_instruction(char command)
{
	...
	gpio_set_value(gpbank->LCD_DB7_PIN_NUMBER, db7_data);
	gpio_set_value(gpbank->LCD_DB6_PIN_NUMBER, db6_data);
	gpio_set_value(gpbank->LCD_DB5_PIN_NUMBER, db5_data);
	gpio_set_value(gpbank->LCD_DB4_PIN_NUMBER, db4_data);

	// Set to command mode
	gpio_set_value(gpbank->LCD_RS_PIN_NUMBER, RS_COMMAND_MODE);
	usleep_range(5, 10);

	// Simulate falling edge triggered clock
	gpio_set_value(gpbank->LCD_E_PIN_NUMBER, 1);
	usleep_range(5, 10);
	gpio_set_value(gpbank->LCD_E_PIN_NUMBER, 0);
}
...
static void lcd_data(char data)
{
	...
	gpio_set_value(gpbank->LCD_RS_PIN_NUMBER, RS_DATA_MODE);
	usleep_range(5, 10);
...
static ssize_t lcd_write_line2(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	int ret;
	char d[256];
	...
	if(ret < 0)
		return -EINVAL;
	lcd_print(d,LCD_SECOND_LINE);
	return count;
}
static ssize_t lcd_write_line1(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	int ret;
	char d[256];
	pr_info("%s: invoked from sysfs...\n",__func__);
...
static ssize_t lcd_clear(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	int ret;
	int d;
	ret=sscanf(buf,"%d\n",&d);
...
static struct kobj_attribute lcd_attr       =__ATTR(Writedata, 0220, NULL, lcd_write);
static struct kobj_attribute cleardata_attr =__ATTR(Cleardata, 0220, NULL, lcd_clear);
static struct kobj_attribute line1_attr     =__ATTR(Write_in_line1, 0220, NULL, lcd_write_line1);
static struct kobj_attribute line2_attr     =__ATTR(Write_in_line2, 0220, NULL, lcd_write_line2);
...
static int lcd_probe(struct platform_device *pdev)
{
	...
	kobj = kobject_create_and_add("lcd", NULL);
	if(!kobj)
                return -ENOMEM;
