...
static void lcd_pin_setup_All(void)
{
	...
	lcd_pin_setup(gpio2bank_base,gpbank->LCD_RS_PIN_NUMBER);
	lcd_pin_setup(gpio2bank_base,gpbank->LCD_E_PIN_NUMBER);
	
	lcd_pin_setup(gpio1bank_base,gpbank->LCD_DB5_PIN_NUMBER);
	lcd_pin_setup(gpio2bank_base,gpbank->LCD_DB4_PIN_NUMBER);
}
...
static void lcd_instruction(char command)
{
	...
	gpio_set_val(gpio2bank_base,gpbank->LCD_DB4_PIN_NUMBER,db4_data);


	// Set to command mode
	gpio_set_val(gpio2bank_base,gpbank->LCD_RS_PIN_NUMBER, RS_COMMAND_MODE);
	usleep_range(5, 10);

	// Simulate falling edge triggered clock
	gpio_set_val(gpio2bank_base,gpbank->LCD_E_PIN_NUMBER, 1);
	usleep_range(5, 10);
	gpio_set_val(gpio2bank_base,gpbank->LCD_E_PIN_NUMBER, 0);
}
...
static void lcd_data(char data)
{
	...
	gpio_set_val(gpio2bank_base,gpbank->LCD_RS_PIN_NUMBER, RS_DATA_MODE);
	usleep_range(5, 10);
...
static int get_gpiobank_address_maping(void)
{
	...
	gpio2bank_base = ioremap(GPIO2_BASE, 4095);
	if(!gpio2bank_base)
		return -1;
	pr_info("GPIO2 Base address as seen by kernel: %p\n", gpio2bank_base);
...
static int get_clockmodule_addr_mapping(void)
{
	...
	writel_relaxed(CM_PER_EN,clock_module_base + CM_PER_GPIO2);
	data = readl_relaxed(clock_module_base + CM_PER_GPIO2);
	pr_info("Clock data %x\n",data);
	data = data | (1 << 18);
	pr_info("after Clock data %x\n",data);
	writel_relaxed(data,clock_module_base + CM_PER_GPIO2);
	return 0;
}
